...
#define GPIO_R_PIN 534
#define GPIO_G_PIN 535
#define GPIO_B_PIN 536
#define DRIVER_NAME "led_rgb_driver"
...
static ssize_t led_write(struct file *File, const char *user_buffer, size_t count, loff_t *offs)
{
	...
	{
		case '0':
			gpio_set_value(GPIO_R_PIN, 0);
			break;
		case '1':
			gpio_set_value(GPIO_R_PIN, 1);
			break;
		case '2':
			gpio_set_value(GPIO_G_PIN, 0);
			break;
		case '3':
			gpio_set_value(GPIO_G_PIN, 1);
			break;
		case '4':
			gpio_set_value(GPIO_B_PIN, 0);
			break;
		case '5':
			gpio_set_value(GPIO_B_PIN, 1);
			break;
		default:
...
static int __init led_init(void)
{
	...
	/** GPIO init **/
	if(gpio_request(GPIO_R_PIN, "GPIO_R_PIN")) {
		printk("Can not allocate GPIO %d\n", GPIO_R_PIN);
		goto led_dev_add;
	}

	/** Set GPIO direction **/
	if(gpio_direction_output(GPIO_R_PIN, 0)) {
		printk("Can not set GPIO %d to output!\n", GPIO_R_PIN);
		goto led_r_gpio;
	}

	/** GPIO init **/
	if(gpio_request(GPIO_G_PIN, "GPIO_G_PIN")) {
		printk("Can not allocate GPIO %d\n", GPIO_G_PIN);
		goto led_dev_add;
	}

	/** Set GPIO direction **/
	if(gpio_direction_output(GPIO_G_PIN, 0)) {
		printk("Can not set GPIO %d to output!\n", GPIO_G_PIN);
		goto led_g_gpio;
	}
	
	/** GPIO init **/
	if(gpio_request(GPIO_B_PIN, "GPIO_B_PIN")) {
		printk("Can not allocate GPIO %d\n", GPIO_B_PIN);
		goto led_dev_add;
	}

	/** Set GPIO direction **/
	if(gpio_direction_output(GPIO_B_PIN, 0)) {
		printk("Can not set GPIO %d to output!\n", GPIO_B_PIN);
		goto led_b_gpio;
	}
...
led_g_gpio:
	gpio_free(GPIO_G_PIN);
led_r_gpio:
	gpio_free(GPIO_R_PIN);
led_dev_add:
